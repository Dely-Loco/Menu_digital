...
export default function WhatsAppButton({ 
  ...
  useEffect(() => {
    // Mostrar el botón después de un breve delay para una entrada suave
    const timer = setTimeout(() => setIsVisible(true), 1000);
    return () => clearTimeout(timer);
  }, []);

  useEffect(() => {
    // Auto mostrar tooltip la primera vez después de 3 segundos
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 ${className}`}>
      ...
      <div className={`
        absolute bottom-full right-0 mb-4 transform transition-all duration-500 ease-out
        ${showTooltip || isExpanded ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-2 scale-95 pointer-events-none'}
      `}>
        <div className="relative">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200/50 p-4 max-w-xs backdrop-blur-sm">
      ...
      <div className="relative group">
        ...
        <Link
          ...
          className="relative block w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-green-400 via-green-500 to-green-600 rounded-full shadow-2xl hover:shadow-green-500/25 transition-all duration-300 ease-out transform hover:scale-110 hover:-translate-y-1 focus:outline-none focus:ring-4 focus:ring-green-400/50 group"
